  ...
  setup(__props) {
    ...
    return (_ctx, _push, _parent, _attrs) => {
      ...
      ssrRenderList(__props.shuttlecock.players, (player) => {
        ...
        ])}"><span class="text-[10px]">✓</span></button><span class="${ssrRenderClass({ "line-through text-gray-400": player.isPaid, "text-gray-700 font-medium": !player.isPaid })}">${ssrInterpolate(player.memberName)}</span></div><button class="text-gray-400 hover:text-red-500 transition-colors"><span class="text-lg leading-none">×</span></button></div>`);
      });
      _push(`<!--]-->`);
  ...
  setup(__props) {
    ...
    return (_ctx, _push, _parent, _attrs) => {
      ...
      if (unref(tubes).length === 0) {
        _push(`<div class="bg-white border-2 border-dashed border-gray-200 rounded-2xl p-16 text-center"><div class="w-20 h-20 bg-gray-50 rounded-full flex items-center justify-center mx-auto mb-6"><span class="text-4xl text-gray-400">🏸</span></div><h2 class="text-xl font-bold text-gray-900 mb-2">No tubes created yet</h2><p class="text-gray-500 mb-8 max-w-sm mx-auto">Create your first shuttlecock tube to start tracking usage and collecting fees.</p><button class="bg-indigo-600 text-white px-8 py-3 rounded-xl font-bold hover:bg-indigo-700 transition-all shadow-lg active:scale-95"> Create First Tube </button></div>`);
      } else {
        _push(`<!---->`);
      }
      _push(`</div></div>`);
...
//# sourceMappingURL=tubes-Cr7Li3nW.js.map
